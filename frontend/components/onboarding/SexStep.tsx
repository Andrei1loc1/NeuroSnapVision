...
import { User, UserCircle, Smile } from "lucide-react";
...
const OPTIONS = [
  { id: "male", label: "Bărbat", icon: User },
  { id: "female", label: "Femeie", icon: UserCircle },
  { id: "other", label: "Altul", icon: Smile },
] as const;

export default function SexStep({ value, onChange, onNext }: SexStepProps) {
  return (
    <section className="mx-6 mt-4 flex flex-col items-center">
      <h2 className="text-2xl font-semibold text-zinc-900 text-center">Care este sexul tău?</h2>
      <p className="text-sm text-zinc-400 mt-1 text-center">Folosit pentru estimarea metabolismului</p>
        ...
        {OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-lg font-semibold ${selected ? "text-emerald-700" : "text-zinc-700"}`}>
                {opt.label}
              </span>
            </button>
          );
